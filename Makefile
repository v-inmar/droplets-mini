...
EVENTDB_NETWORK := eventdb_network
HISTORYDB_NETWORK := historydb_network

.PHONY: webup-network webup-storage webup-todo webup-history webup-gateway webup webdown webstatus webclean webup-event

# -- Individual / multi-terminal --

webup-network:
	docker network inspect $(EVENT_NETWORK) >/dev/null 2>&1 || docker network create $(EVENT_NETWORK)
	docker network inspect $(EVENTDB_NETWORK) >/dev/null 2>&1 || docker network create $(EVENTDB_NETWORK)
...
webup-event:
	docker compose -f services/event/docker-compose.yml build --no-cache
	docker compose -f services/event/docker-compose.yml up
...
webdown:
	docker compose -f services/event/docker-compose.yml down
...
webstatus:
	...
	docker compose -f services/event/docker-compose.yml ps
...
webclean: webdown
	...
	docker compose -f services/event/docker-compose.yml down --rmi local
